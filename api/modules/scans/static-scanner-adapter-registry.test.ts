...
describe("StaticScannerAdapterRegistry", () => {
  it("registers the external Semgrep adapter without treating it as core", () => {
  ...
  it("keeps the Semgrep adapter frozen and externally attributed", () => {
    const registry = createStaticScannerAdapterRegistry({
  ...
  it("preserves Semgrep diff targeting and technology rule contributions", async () => {
    ...
    expect(resolveStaticScannerDiffExecution(adapter, ["src/app.ts"])).toEqual({
  ...
  it("rejects an adapter that broadens another binary invocation policy", () => {
    ...
    expect(() => new StaticScannerAdapterRegistry().register(adapter)).toThrow(
      "Conflicting Docker scanner invocation policy: gitleaks",
    );
  });
});
