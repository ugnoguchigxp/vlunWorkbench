...
async function main() {
	...
	if (!trivyVersion) {
		writeResult({
			ok: false,
			status: "failed",
			message: "Trivy executable not found",
		});
		dbConnection.sqlite.close(false);
		process.exit(1);
	}
	...
	let trivyExitCode: number | null = null;
	let findingCount = 0;
	let evidenceCount = 0;
	...
	try {
		await scanRepo.createScanEvent({
			...
			message: `Trivy scan started for project: ${project.name}`,
		});

		const toolRun = await scanRepo.createToolRun({
			scanRunId: scanRun.id,
			toolName: "trivy",
			toolVersion: trivyVersion,
			status: "running",
			command: "trivy fs",
		});
		...
		trivyExitCode = runResult.exitCode;
		...
		if (!runResult.ok) {
			throw new Error(
				runResult.error ||
					`Trivy run failed with exit code ${runResult.exitCode}`,
			);
		}
		...
		await scanRepo.createScanEvent({
			...
			message: "Parsing Trivy raw output.",
		});

		const normalizedFindings = normalizeTrivy(runResult.rawJson, {
		...
		await scanRepo.createScanEvent({
			...
			message: `Successfully parsed ${normalizedFindings.length} findings from Trivy output.`,
		});
		...
		await scanRepo.updateScanRunStatus(scanRun.id, "completed", {
			summary: `Trivy scan completed successfully. Found ${findingCount} findings.`,
		});
